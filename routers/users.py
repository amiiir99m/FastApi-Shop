...
from schema.input import RegisterInput, UpdateUserProfileInput, UserInput
...
async def register(
    ...
    user = await UsersOperation(db_session).create(
        username=data.username, password=data.password, role=data.role) #type:ignore
    return user
...
async def update_user_profile(
    ...
    user = await UsersOperation(db_session).update_username(token_data.username, data.new_password)
